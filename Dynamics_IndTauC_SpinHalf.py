...
def DynamiteAvg(tau, tauPi, eps, ppmP1, ppmNV, K, cycles): 
    print("tau: %.2f"%tau, end= ",  ")
    print("eps: %.2f"%eps, end = ", ")
    print("NP1sStart: %d"%NP1sStart)
    print("------")
    ...
    for k in range(K):
        ...
        tauCs, to_remove = GetTauC(rsP1s, EstimateGammaD(ppmP1), nuclear = "Spin1/2" )
        #print(tauCs)
        #print(len(tauCs))
        #print(np.min(tauCs))
        ...
        NP1s = np.shape(rsP1s)[1]
        print("NP1s before/after: ", NP1sStart, " / ", NP1s, end = ",  ")        
        ...
        print("eps_tau: %.2f"%eps_tau, end = "  " )
        print("eps_spatial: %.2f"%eps_spatial, end = "  " )
        print("eps_stability: %.2f"%eps_stability, end = "  " )
        #print("tauCs: ", tauCs)
...
outFile = params["outDir"] + "/Bauch_SpinHalfNitrogen_CMPG_"
